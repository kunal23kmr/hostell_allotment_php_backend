...
if(isset($_POST['submit']))
{
 ...
 //   $row=mysqli_fetch_array($result);
    if($_SESSION['CAPTCHA_CODEGH2']==$captcha)
    {
    $hostel_no="gh1";
    if($hostel_no=="gh1")
    {   $checksql="select vacant_seats from gh1details where room_no='$room_no'";
        $checkresult=mysqli_query($con,$checksql);
        $checkdata=mysqli_fetch_array($checkresult);
        
        if($checkdata['vacant_seats']>0)
        {
          $seat_update_sql="update gh1details set vacant_seats=vacant_seats-1, filled_seats=filled_seats+1 where room_no='$room_no'";
  ...
  {
      echo '<script type=\'text/javascript\'> alert("No Vacant Rooms In the Room! Please Try Again!"); location="gh2.php";</script>';
  }
...
{
    echo '<script type=\'text/javascript\'> alert("Wrong Captcha Code Please Select a Seat Again!"); location="gh2.php";</script>';
}
